...
An AI-driven resume *analyzer*, not a resume builder/template tool and not (yet) an auto-apply or job-matching engine. The mechanism a neighboring product can't casually copy: structured, on-demand scoring (score + summary + itemized findings) generated by an LLM (Claude via AWS Bedrock) against the user's actual resume content and a specific job, not generic advice.
